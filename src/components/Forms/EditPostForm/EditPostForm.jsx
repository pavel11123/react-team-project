...
const EditPostForm = ({ image, text, title, tags, _id, handleClose }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
          const newPost = await api.editPost(token, 
            { image: data.image, tags: data.tags.split(","), text: data.text, title: data.title}, _id
          );
     ...
     <Stack className={s.icon} direction="row" justifyContent="flex-end">
      ...
      <div className={s.wrapper}>
         <form className={s.form} onSubmit={handleSubmit(onSubmit)}>
            <Typography className={s.title} variant="h4" color="title">Редактировать пост</Typography>
