...
import { REGEXP_EMAIL, VALIDATE_MESSAGE, REGEXP_PASSWORD } from "../../../utils/constants";
...
const ResetPasswordForm = ({ setActiveModal }) => {
    ...
    const sendData = async (data) => {
        ...
        console.log({ data });
        try {
            const res = await api.changePass(data.token, {password: data.password});
            console.log({res});
    ...
    return (
        ...
        <form className={s.form} onSubmit={handleSubmit(sendData)}>
            ...
            <span className={cn(s.links, s.resetPassword)} onClick={()=>navigate(-1)}>Назад</span>
